...
export function testPropagation(config: IssuePropagationConfig, context: Omit<PropagationContext, 'issues'>): void {
    ...
    console.log("avg f1", testResults.map((result) => result.f1).reduce((acc, curr) => acc + curr) / testResults.length);
    // print averaged precision
    console.log("avg precision", testResults.map((result) => result.precision).reduce((acc, curr) => acc + curr) / testResults.length);
    // print averaged recall
    console.log("avg recall", testResults.map((result) => result.recall).reduce((acc, curr) => acc + curr) / testResults.length);
